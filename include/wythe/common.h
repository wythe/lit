...
inline bool starts_with(std::string_view src, std::string_view value)
{
...
template <typename T> inline std::vector<char> read_file(const T &filename)
{
	...
	if (file.gcount() != sz)
		PANIC("read " << file.gcount() << " bytes, expected " << sz);
...
inline void write_file(T first, T last, const std::string name)
{
	...
	if (!s)
		PANIC("Can't open file \"" << n << "\" for writing.");
	s.write(&(*first), last - first);
}

// TODO: remove this version
template <typename S, typename T> inline void write_file(S name, const T &data)
{
...
struct url {
	...
	void str(std::ostream &os) const { os << path << file << anchor; }
...
{
	return (x.path == y.path) && (x.file == y.file) &&
	       (x.anchor == y.anchor);
}
...
inline bool bit_is_set(unsigned char byte, int bit_index)
{
	...
	PANIC("invalid bit index for byte: " << bit_index);
}
...
inline std::string &to_hex_string(T first, T last, std::string &dest)
{
	static char const *hmmmm[] = {
	    ...
	    "DE", "DF", "E0", "E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8",
...
// copy a string.  If truncate false, throw an exception if size_dest is too
// small, else truncate.
inline int safe_str_copy(char *dest, size_t size_dest, const char *src,
			 size_t n, bool truncate = false)
{
	if (size_dest <= n) {
		if (truncate)
			n = size_dest - 1;
